...
class ControllingAction(object):
    ...
    def __init__(self):
        ...
        log_msg = (f'`{anm.ACTION_CONTROLLER}` Action Server initialised. It will navigate trough the plan with a delay ' 
                   f'between each location spanning in [{self._random_motion_time[0]}, {self._random_motion_time[1]}).')
    ...
    def execute_callback(self, goal):
        ...
        if goal is None or goal.plan is None or len(goal.plan) == 0:
            rospy.logerr(anm.tag_log('No locations provided! This service will be aborted!', LOG_TAG))
        ...
        for location in goal.plan:
            ...
            _set_pose_client(location)
            # Log current robot position.
            log_msg = f'Reaching location ({location}).'
            rospy.loginfo(anm.tag_log(log_msg, LOG_TAG))
